...
/**
 * Drives the primary environment's Tung Tung Tunnel link. Tung Tung Tunnel (managed
...
export function useCloudLinkController() {
  ...
  const reportUpdateFailure = (cause: unknown) => {
    const message =
      cause instanceof Error ? cause.message : "Could not update Tung Tung Tunnel access.";
    const traceId = findErrorTraceId(cause);
    console.error("[t3-connect] Could not update Tung Tung Tunnel", { message, traceId, cause });
    setOperationError(traceId ? `${message} Trace ID: ${traceId}` : message);
    toastManager.add({
      type: "error",
      title: "Could not update Tung Tung Tunnel",
      description: message,
      data: traceId
        ? {
  ...
  const reconcileCloudState = async (desired: CloudLinkDesiredState): Promise<boolean> => {
    ...
    if (!wantsLink) {
      ...
      // leave the user unable to turn Tung Tung Tunnel off.
      ...
      if (!clerkToken) {
        reportUpdateFailure(new Error("Sign in to Tung Tung Tunnel before enabling this."));
        return false;
      }
